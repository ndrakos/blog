...
$$\rho_(R,z) = \dfrac{M_d}{4 \pi h^2 z_0} \exp(-R/h) {\rm sech}^2 \left(\dfrac{z}{z_0} \right)$$
...
$$P(<R) = \dfrac{\int_{-\infty}^\infty \int_0^R \rho R' dR' dz'}{\int_0^\infty \int_0^\infty \rho R' dR' dz'}$$
...
$$P(<z) = \dfrac{2 \pi \int_{-\infty}^z \int_0^\infty \rho R' dR' dz'}{2 \pi \int_{-\infty}^\infty \int_0^\infty \rho R' dR' dz'}$$
...
$$\Phi(R,\phi) = - G {\rm d}R' R' \int_0^\infty {\rm d}R' \int_0^{2 \pi} {\rm d} \phi'\dfrac{\Sigma(R',\phi')}{\sqrt{R'^2 + R^2 - 2 R R' \cos(\phi'-\phi)}}$$
...
To test this, I evolved $$10^5$$ disk particles in isolation (no background potential), with the parameters $$Md = 1$$, $$h = 1$$, $$z_0 = 0.2$$, $$G = 1$$, $$Q = 1.5$$, $$R_{\rm ref}= 8.5/3.5$$


**STILL TROUBLESHOOTING... GADGET IS CRASHING**
