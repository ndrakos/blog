...
Then, I only consider (sub)halos with $${\rm RA}<1$$ and $${\rm Dec}<1$$ degree. Further, I only take halos with $$d<60*115$$; this is because the lightcone is not complete for distances larger than this (this doesn't actually make a difference, because there aren't halos out that far). Finally, I recenter the survey by rotating the $$(x,y,z)$$ coordinates by angles $$(\psi,\phi,\theta) = (-0.5,0,0.5)$$; now, the x-axis corresponds to the distance from the observer.
